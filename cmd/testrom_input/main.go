...
func main() {
	...
	// Helper for BGT offset (Branch if Greater Than - signed)
	bgt := func(offset int16) {
		add(0xC300)
		addImmVal(uint16(offset))
	}
	...
	// Read controller state (8-bit read from I/O)
	// Read low byte (CONTROLLER1 at 0xA000 maps to input offset 0x0000)
	movImm16(7, 0xA000)                   // R7 = CONTROLLER1 (low byte)
	add(0x1000 | (2 << 8) | (2 << 4) | 7) // MOV R2, [R7] (8-bit read, zero-extended)
	// R2 now contains low byte of button state (bits 0-7: UP, DOWN, LEFT, RIGHT, A, B, X, Y)
	...
	// R2 now contains the button state (low byte, zero-extended to 16-bit)
	// If no buttons are pressed, R2 should be 0
	...
	// ============================================
	// Wrap X position (0-319)
	// ============================================
	// Check if X == 65535 (unsigned wrap from 0 when going left)
	movReg(3, 0)     // R3 = X
	cmpImm(3, 65535) // CMP R3, #65535
	wrapXLeftPC := len(code) * 2
	bne(0)           // BNE skip_wrap_x_left (if X != 65535, skip)
	movImm16(0, 319) // MOV R0, #319 (wrap to right edge)
	wrapXLeftTarget := len(code) * 2
	// Fix branch offset
	code[(wrapXLeftPC/2)+1] = uint16(calcOffset(wrapXLeftPC+2, wrapXLeftTarget))

	// Check if X > 319 (X >= 320 when going right off-screen)
	// Use signed comparison: if X > 319, wrap to 0
	// Note: 65535 as signed is -1, so BGT won't catch it (already handled above)
	movReg(3, 0)   // R3 = X
	cmpImm(3, 319) // CMP R3, #319
	wrapXRightPC := len(code) * 2
	bgt(0) // BGT wrap_x_right (if X > 319, wrap to 0)
	wrapXRightSkip := len(code) * 2
	jmp(0) // JMP skip_wrap_x_right (placeholder)
	wrapXRightTarget := len(code) * 2
	// Wrap to 0 when going right off-screen
	movImm16(0, 0) // MOV R0, #0 (wrap to 0)
	wrapXRightEnd := len(code) * 2
	wrapXRightSkipTarget := len(code) * 2
	// Fix branch offsets
	code[(wrapXRightPC/2)+1] = uint16(calcOffset(wrapXRightPC+2, wrapXRightTarget))
	code[(wrapXRightSkip/2)+1] = uint16(calcOffset(wrapXRightSkip+2, wrapXRightSkipTarget))
	code[(wrapXRightEnd/2)+1] = uint16(calcOffset(wrapXRightEnd+2, wrapXRightSkipTarget))

	// ============================================
	// Wrap Y position (0-199, wrap to 0 if >= 200)
	// ============================================
	// Check if Y >= 200 (screen height)
	movReg(3, 1)   // R3 = Y
	cmpImm(3, 200) // CMP R3, #200
	wrapYPC := len(code) * 2
	bne(0)         // BNE skip_wrap_y (if Y < 200, skip wrap)
	movImm16(1, 0) // MOV R1, #0 (wrap to 0)
	wrapYTarget := len(code) * 2
	// Fix branch offset: PC of immediate is wrapYPC + 2
	code[(wrapYPC/2)+1] = uint16(calcOffset(wrapYPC+2, wrapYTarget))
