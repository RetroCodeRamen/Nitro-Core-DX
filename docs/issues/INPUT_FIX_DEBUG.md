...
1. **Run with logging**:
   ```bash
   ./nitro-core-dx -rom roms/input_test.rom -log
   ```
...
1. Rebuild the test ROM: `go build -o testrom_input ./cmd/testrom/input && ./testrom_input roms/input_test.rom`
2. Rebuild the emulator: `go build -tags "no_sdl_ttf" -o nitro-core-dx ./cmd/emulator`
3. Run again: `./nitro-core-dx -rom roms/input_test.rom`
