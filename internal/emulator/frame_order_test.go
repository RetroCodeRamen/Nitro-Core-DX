...
func TestFrameCounterAdvancesOncePerExactFrameStep(t *testing.T) {
	emu := NewEmulator()
	cyclesPerFrame := uint64(ppu.TotalScanlines * ppu.DotsPerScanline)

	if err := emu.PPU.StepPPU(cyclesPerFrame); err != nil {
		t.Fatalf("first StepPPU error: %v", err)
	}
	if got := emu.PPU.FrameCounter; got != 1 {
		t.Fatalf("after first exact frame step: got FrameCounter=%d, want 1", got)
	}

	if err := emu.PPU.StepPPU(cyclesPerFrame); err != nil {
		t.Fatalf("second StepPPU error: %v", err)
	}
	if got := emu.PPU.FrameCounter; got != 2 {
		t.Fatalf("after second exact frame step: got FrameCounter=%d, want 2", got)
	}
}
