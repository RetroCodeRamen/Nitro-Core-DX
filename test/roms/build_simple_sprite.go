...
func main() {
	...
	// Calculate offset AFTER BNE instruction is added (PC points to offset word)
	...
	// Infinite loop - reset OAM_ADDR to 0 before looping
	mainLoopStart := uint16(builder.GetCodeLength() * 2)
	
	// Reset OAM_ADDR to 0 (so sprite doesn't advance on next iteration)
	builder.AddInstruction(rom.EncodeMOV(1, 4, 0)) // MOV R4, #0x8014 (OAM_ADDR)
	builder.AddImmediate(0x8014)
	builder.AddInstruction(rom.EncodeMOV(1, 5, 0)) // MOV R5, #0x00 (sprite 0)
	builder.AddImmediate(0x00)
	...
	// Loop back to start (will rewrite sprite 0 data)
