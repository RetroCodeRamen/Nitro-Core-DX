...
func main() {
	...
	// Simple infinite loop - just NOP forever
	// NOP
	builder.AddInstruction(rom.EncodeNOP())
	
	// JMP back to the NOP (jump back 4 bytes: 2 for NOP + 2 for JMP + 2 for immediate)
	// Offset is relative to PC after JMP+immediate, so -6 bytes = -3 words = 0xFFFD
	builder.AddInstruction(rom.EncodeJMP())
	builder.AddImmediate(0xFFFD) // Jump back to NOP
