...
func main() {
	...
	// ============================================
	// Palette 0, Color 0 = Gray (initial visible color, changes based on input)
	// Start with gray so we can see if the ROM is working
	...
	// Gray: RGB555 = 0x4210 (medium gray)
	builder.AddInstruction(rom.EncodeMOV(1, 5, 0)) // MOV R5, #0x10 (low byte)
	builder.AddImmediate(0x10)
	builder.AddInstruction(rom.EncodeMOV(3, 4, 5)) // MOV [R4], R5
	builder.AddInstruction(rom.EncodeMOV(1, 5, 0)) // MOV R5, #0x42 (high byte)
	builder.AddImmediate(0x42)
	builder.AddInstruction(rom.EncodeMOV(3, 4, 5)) // MOV [R4], R5
	...
	// ============================================
	// Set up tilemap at 0x4000 BEFORE enabling BG0
	// Set VRAM address to tilemap base (0x4000)
	...
	// This will fill the screen with tile 0, which uses color index 0 from palette 0
	...
	// Enable BG0 AFTER tilemap is set up
	...
	// Wait for VBlank before updating display
	...
	vblankPC := uint16(builder.GetCodeLength() * 2)
	vblankOffset := rom.CalculateBranchOffset(vblankPC, waitVBlankStart)
	builder.AddImmediate(uint16(vblankOffset))
